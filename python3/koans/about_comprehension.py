...
class AboutComprehension(Koan):
    ...
    def test_filtering_lists_with_list_comprehensions(self):
        ...
        self.assertEqual(5, len(feast)) # 5
        self.assertEqual(3, len(comprehension)) # 3 are longer than 6

    def test_unpacking_tuples_in_list_comprehensions(self):
        list_of_tuples = [(1, 'lumberjack'), (2, 'inquisition'), (4, 'spam')]
        comprehension = [ skit * number for number, skit in list_of_tuples ]

        self.assertEqual('lumberjack', comprehension[0]) # number = 1 skit = 'lumberjack' - 1 * lumberj = 1 lumberj
        self.assertEqual('spamspamspamspam', comprehension[2]) # 2 is spam list - spam * 4
    ...
    def test_double_list_comprehension(self):
        ...
        self.assertEqual(6, len(comprehension)) # not clear
